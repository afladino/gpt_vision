...
class GPTVision():
  ...
  async def fetch_gptv(self,bs64img_list,file_name,session):
    '''
  ...
  async def models_caller(self,multiple_imgs,file_name,save):
      ...
      async with aiohttp.ClientSession() as session:
        tasks = [self.fetch_gptv(imgPack["images"],file_name,session) for imgPack in multiple_imgs]
...
def main():
  start = time.time()
  fileName = "metalmecanica_5596_240x240.png"
  tablero = "Tablero_4_240x240.png"
  #* Path to image
  ...
  model=GPTVision()
  asyncio.run(model.models_caller(multiple_imgs=cotizacion,file_name=fileName,save=False))
  finish = time.time()
  timeEx = finish - start
